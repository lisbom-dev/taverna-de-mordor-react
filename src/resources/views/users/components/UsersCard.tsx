...
const UsersCard: React.FC<IUserCardProps> = ({ name, email, cell_phone_number, instagram_ref, is_adm, is_master, photo_url}) => {
  return (
    <>
        <tr>
        ...
        <td className="px-5 py-5 bg-white text-sm">
            <a href={`https://api.whatsapp.com/send?phone=${cell_phone_number}`} target="_blank">
        ...
        <td className="px-5 py-5 bg-white text-sm">
            {instagram_ref? 
            <a
                href={`http://instagram.com/${instagram_ref}`}
                target="_blank"
            >
                <p className="text-blue-500 whitespace-no-wrap">
                {instagram_ref}
